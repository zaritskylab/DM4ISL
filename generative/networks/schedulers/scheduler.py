...
def _cosine_beta(num_train_timesteps: int, s: float = 8e-3):
    ...
    for i in range(num_train_timesteps):
        ...
        alpha_cumprod_t1  = np.cos((t1 + s) / (1 + s) * np.pi / 2) ** 2
        alpha_cumprod_t2  = np.cos((t2 + s) / (1 + s) * np.pi / 2) ** 2
        betas.append(min(1 - alpha_cumprod_t2 / alpha_cumprod_t1, 0.999))
    betas = torch.tensor(np.array(betas))
    betas = torch.clip(betas, 0.0001, 0.9999)
    alphas = 1 - betas
    alphas_cumprod = torch.cumprod(alphas, dim=0)
    
    return betas, alphas, alphas_cumprod # alphas_cumprod[:-1]
...
class Scheduler(nn.Module):
    ...
    def __init__(self, num_train_timesteps: int = 1000, schedule: str = "linear_beta", **schedule_args) -> None:
        ...
        if isinstance(noise_sched, tuple):
            ...
            # alphas_bar_sqrt = alpha_prod_t.sqrt()
            # alphas_bar_sqrt_0 = alphas_bar_sqrt[0].clone()
            # alphas_bar_sqrt_T = alphas_bar_sqrt[-1].clone()
            # alphas_bar_sqrt -= alphas_bar_sqrt_T # # Shift so the last timestep is zero.
            # alphas_bar_sqrt *= alphas_bar_sqrt_0 / (alphas_bar_sqrt_0 - alphas_bar_sqrt_T) # Scale so the first timestep is back to the old value.
            # alphas_bar = alphas_bar_sqrt**2
            # self.alphas_cumprod = alphas_bar
